...
const tools = [
  ...
  },
  {
    ...
    description:
      "Create a clean Word .docx study document from structured Markdown or raw summary text generated by DeepSeek. Use this in the same workflow that creates the summary, not from memory after printing text to the terminal.",
    inputSchema: {
      type: "object",
      properties: {
        title: {
        ...
        content_markdown: {
          type: "string",
          description: "Structured Markdown or raw terminal summary text to write into the Word document. The converter normalizes headings, lists, tables, bold text, and code spans.",
        },
        mindmap_mermaid: {
          type: "string",
...
function makeWordSummaryPrompt(args = {}) {
  ...
  return [
    ...
    "If the user asks for a Word file, do not only print the notes in the terminal. After drafting the final Markdown, immediately call `courseware.create_study_docx` with the exact same Markdown.",
    "",
    "Required structure:",
    "# Title",
    "## 1. Course Overview",
    ...
    "- Use clean Markdown headings, bullets, numbered lists, and tables so the DOCX converter can preserve structure.",
  ].join("\n");
}
